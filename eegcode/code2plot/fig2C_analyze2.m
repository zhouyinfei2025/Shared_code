%% Time domain average of SSVEPs
% Time domain average of SSVEPs to high-prob dist-loc [baseline, learning]
% Time domain difference of SSVEPs to low-prob dist-loc [baseline, learning]
% Difference[high VS.low] of difference[learning-baseline]
% inspect the pattern of each chan
...
%% 
dat2plot_E1_H = deal(zeros(28,59,1626)); %28个被试的数据
dat2plot_E2_H = deal(zeros(28,59,1626));
dat2plot_E1_L = deal(zeros(28,59,1626)); %28个被试的数据
dat2plot_E2_L = deal(zeros(28,59,1626));
...
%high-prob dist-loc
 for subno = 1:length(sublist)
    ...
    dat2plot = squeeze(tfdata(:,:,f2plot,t2plot(1):t2plot(2))); 
      
    %% Save the data
    if strcmp(readdir,E1) 
        dat2plot_E1_H(subno,:,:) = dat2plot; 
    else
        dat2plot_E2_H(subno,:,:) = dat2plot;
    end
end   
...
for subno = 1:length(sublist)
    ...
    %% Save the data
    if strcmp(readdir,E1) 
        dat2plot_E1_L(subno,:,:) = dat2plot; 
    else
        dat2plot_E2_L(subno,:,:) = dat2plot;
    end
end   
...
for chani = 1:57          
    ...
    X1 = squeeze(dat2plot_E1_H(:,chani,:));
    X2 = squeeze(dat2plot_E2_H(:,chani,:));
...
for chani = 1:57
    ...
    X1 = squeeze(dat2plot_E1_L(:,chani,:));
    X2 = squeeze(dat2plot_E2_L(:,chani,:));
...
for chani = 1:57
    ...
    X1 = squeeze(dat2plot_E2_H(:,chani,:) - dat2plot_E1_H(:,chani,:)); % 高概率干扰位置的学习-基线
    X2 = squeeze(dat2plot_E2_L(:,chani,:) - dat2plot_E1_L(:,chani,:)); % 低概率干扰位置的学习-基线
    ...
    axes(ha(chani))
    set(legend,'AutoUpdate','off')
    plot(tftime,squeeze(mean(X1)),'Color','r');hold on
    plot(tftime,squeeze(mean(X2)),'Color','k');
    shadedErrorBar(tftime,squeeze(mean(X1)),std(X1)/sqrt(nSubjects-1),'r','0.2');hold on
    shadedErrorBar(tftime,squeeze(mean(X2)),std(X2)/sqrt(nSubjects-1),'k','0.2');hold on
    line([1250 1250],[min(ylim) max(ylim)],'LineStyle','--','Color','black')
    line([min(xlim) max(xlim)],[0 0],'LineStyle','--','Color','black')
    yl=get(gca,'ylim');
    set(gca,'xlim',[0 3250]);
    set(gca,'Box','off')
    title(chanlist(chani))
...
saveas(figure(1),[savedir filesep 'high.png']) 
saveas(figure(2),[savedir filesep 'low.png']) 