...
if singletrial
    ...
    z_pow = cell(size(eegdat));
    tf_phase = NaN;
elseif robfit
    tf_rvals = zeros(nconds,length(channels),nfreqs,length(times2saveidx),size(regressors{1},2));
else
    [raw_pow, tf_pow, tf_phase] = deal(zeros(nconds,length(channels),nfreqs,length(times2saveidx)));
end
...
for condi=1:nconds
    ...
    if singletrial
        ...
        raw_pow = abs(rawconv).^2; % raw_pow: chan*freq*time*trial
        % tf_pow: 直接在频率维度上对 8-12Hz进行平均
        tf_pow{condi} = squeeze(mean(raw_pow(:,5:9,:,:), 2)); 
        % z_pow: 在时间维度上进行 Z 转换后，再对 8-12Hz频率进行平均
        z_raw_pow = zscore(raw_pow, 1, 3);  % Z转换在时间维度上
        z_pow{condi} = squeeze(mean(z_raw_pow(:,5:9,:,:), 2)); 
        
    elseif sum(basetime)
        %tf_pow(condi,:,:,:)   = mean(abs(rawconv).^2,4); % trials average
        %tf_phase(condi,:,:,:) = abs(mean(exp(1i*angle(rawconv)),4));
        raw_pow(condi,:,:,:) = mean(abs(rawconv).^2,4); % trials average
    end
...
if sum(basetime)
    ...
    elseif strcmp(baselinetype,'conavg')
        ...
        tf_pow = 10*log10( raw_pow ./ repmat(mean(baselinedata,1),[ nconds 1 1 length(times2save) ]) );
    end
end
...
if save_output
    ...
    elseif singletrial
        save(outputfilename,'tf_pow','z_pow','dim');
    elseif robfit
        if sum(basetime)
        ...
        save(outputfilename,'raw_pow','tf_pow','dim')
    end
end
